...
class MindMap:
    ...
    def ask_chatgpt_for_explanation(self, selected_node: str) -> None:
        # Construct the prompt for ChatGPT
        conversation = [
            {"role": "system", "content": "You are a knowledgeable AI. Summarize concepts in an understnadable way."},
            {"role": "user", "content": f"Explain the node '{selected_node}' in its context."}
        ]
        ...
        st.sidebar.text_area("Explanation", explanation)

            

        # Return the explanation and the updated conversation
        return explanation, conversation + [Message(explanation, role="assistant")]
    ...
    def _add_expand_delete_buttons(self, node) -> None:
        ...
        cols[0].button(
            label="Explain", 
            on_click=self.ask_chatgpt_for_explanation,
            key=f"explain_{node}",
            # pass to on_click (self.ask_chatgpt_for_explanation)
